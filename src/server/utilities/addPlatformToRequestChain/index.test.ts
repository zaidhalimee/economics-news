...
describe('addPlatformToRequestChain', () => {
  ...
  it('does not remove SIMORGH from the req-svc-chain header if it is not the final service', () => {
    const headers = { 'req-svc-chain': 'SERVICE_1,SIMORGH,SERVICE_2' };
    const result = addPlatformToRequestChain({
      headers,
      application: 'EXPRESS',
    });

    expect(result).toStrictEqual('SERVICE_1,SIMORGH,SERVICE_2,SIMORGH,EXPRESS');
  });
